  ...
  return `<header class="cmd-header cmd-toolbar fade-in">
    ...
    <div class="cmd-header__right">
      ...
      <div class="cmd-header__actions">
        <a href="/income" class="btn btn--sm btn--primary btn--compact">+ Gelir</a>
        <a href="/expense" class="btn btn--sm btn--ghost btn--compact">+ Gider</a>
...
function operationsCenter({ alerts, fuelPct, fuelExpenseTotal }) {
  const topPlate = alerts.topFuel ? escapeHtml(alerts.topFuel.plate) : "—";
  const topAmount = alerts.topFuel ? money(alerts.topFuel.total) : "—";
  const fuel30 = alerts.fuel30;
  const hgs = alerts.hgs || { hasImport: false, latest: null };
  const hgsLatest = hgs.latest;

  const hgsValueHtml = hgs.hasImport && hgsLatest
    ? `<strong class="cmd-ops__value">${money(hgsLatest.passage_total || 0)}</strong>
        <span class="cmd-ops__sub cmd-ops__sub--hgs">${escapeHtml(hgsLatest.plate_normalized || hgsLatest.vehicle_plate || "—")} · ${Number(hgsLatest.passage_count || 0).toLocaleString("tr-TR")} geçiş · ${escapeHtml(hgsLatest.period_label || "Son dönem")}</span>`
  ...
  return `<section class="cmd-panel cmd-panel--ops fade-in">
    <header class="cmd-panel__head">
      <div>
        <h3 class="cmd-panel__title">Yakıt &amp; HGS Operasyon Merkezi</h3>
        <p class="cmd-panel__desc">Son 30 gün operasyon sinyalleri</p>
      </div>
      <a href="/fuel" class="btn btn--ghost btn--sm">Yakıt →</a>
    </header>
    <div class="cmd-panel__body cmd-ops">
      <div class="cmd-ops__stat cmd-ops__stat--fuel">
        <span class="cmd-ops__label">Son 30 Gün Yakıt</span>
        <strong class="cmd-ops__value cmd-ops__value--fuel">${money(fuel30.totalCost)}</strong>
        <span class="cmd-ops__sub">${Number(fuel30.totalLiters).toLocaleString("tr-TR")} L · ${fuel30.count} kayıt</span>
      </div>
      <div class="cmd-ops__stat cmd-ops__stat--hgs">
        <span class="cmd-ops__label">HGS Modülü</span>
        ${hgsValueHtml}
        <a href="/hgs" class="cmd-ops__link cmd-ops__link--hgs">HGS Yönetimi →</a>
      </div>
      <div class="cmd-ops__stat">
        <span class="cmd-ops__label">En çok yakıt gideri</span>
        <strong>${topPlate}</strong>
        <span class="cmd-ops__sub">${topAmount}</span>
        ${alerts.topFuel?.vehicle_id ? `<a href="/vehicle/${alerts.topFuel.vehicle_id}" class="cmd-ops__link cmd-ops__link--fuel">Araç detayı →</a>` : ""}
      </div>
      <div class="cmd-ops__ratio">
        <div class="cmd-ops__ratio-head">
          <span>Yakıt / toplam gider</span>
          <strong>${fuelPct}%</strong>
        </div>
        <div class="progress"><div class="progress__bar progress__bar--fuel" style="width:${fuelPct}%"></div></div>
        <span class="cmd-ops__sub">${money(fuelExpenseTotal)} gider payı</span>
      </div>
    </div>
  </section>`;
}
...
function financialMovementsPanel(transactions) {
  ...
  return `<section class="cmd-panel cmd-panel--movements fade-in">
    <header class="cmd-panel__head">
      ...
      <a href="/income" class="btn btn--ghost btn--sm">Tümü</a>
    </header>
