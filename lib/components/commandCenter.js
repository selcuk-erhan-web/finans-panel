...
const INSURANCE_DOC_TYPES = new Set(["traffic_insurance", "casco", "seat_insurance"]);
const TECHNICAL_DOC_TYPES = new Set(["inspection", "emission"]);
...
function formatNearestCriticalLabel(item) {
  if (!item?.date) return "Yakın tarih bulunmuyor";
  const formatted = new Date(`${item.date.slice(0, 10)}T12:00:00`).toLocaleDateString("tr-TR", {
    day: "numeric",
    month: "short",
    year: "numeric",
  });
  const plate = item.plate ? ` · ${formatPlateDisplay(item.plate) || item.plate}` : "";
  if (item.daysLeft < 0) {
    return `${item.label}${plate} — ${formatted} (${Math.abs(item.daysLeft)} gün geçti)`;
  }
  if (item.daysLeft === 0) {
    return `${item.label}${plate} — ${formatted} (bugün)`;
  }
  return `${item.label}${plate} — ${formatted} (${item.daysLeft} gün)`;
}

function buildUpcomingWorkCenterContext(bundle = {}) {
  const ref = new Date();
  const docKpi = documentService.getKpiSummary(ref);
  const upcomingDocs = documentService.listUpcoming(ref, 100);
  ...
  const muayene = upcomingDocs.filter((row) => TECHNICAL_DOC_TYPES.has(row.document_type)).length;
  const sigorta = upcomingDocs.filter((row) => INSURANCE_DOC_TYPES.has(row.document_type)).length;
  const bakim =
    (scheduleSummary.due || 0) +
    (scheduleSummary.overdue || 0) +
    (scheduleSummary.upcoming || 0);
  const lastik =
    (tireSummary.unknown || 0) +
    (tireSummary.mismatch || 0) +
    (tireSummary.attention || 0);
  const kritikEvrak = (docKpi.expired || 0) + (docKpi.within7 || 0);

  const candidates = [];
  for (const doc of upcomingDocs) {
    if (!doc.expiry_date) continue;
    candidates.push({
      daysLeft: doc.daysLeft ?? daysUntilDate(doc.expiry_date, ref) ?? 9999,
      date: doc.expiry_date,
      label: doc.type_label || "Evrak",
      plate: doc.plate,
    });
  }

  const alerts = bundle.alerts || {};
  for (const row of alerts.upcoming || []) {
    if (!row.next_service_date) continue;
    candidates.push({
      daysLeft: daysUntilDate(row.next_service_date, ref) ?? 9999,
      date: row.next_service_date,
      label: row.type_label || row.maintenance_type || "Bakım",
      plate: row.plate,
    });
  }
  for (const row of alerts.muayeneSigorta || []) {
    if (!row.next_service_date) continue;
    candidates.push({
      daysLeft: daysUntilDate(row.next_service_date, ref) ?? 9999,
      date: row.next_service_date,
      label: row.type === "muayene" ? "Muayene" : row.type === "sigorta" ? "Sigorta" : "Bakım",
      plate: row.plate,
    });
  }

  candidates.sort((a, b) => a.daysLeft - b.daysLeft);
  const nearestCritical = candidates[0] || null;
  ...
  return {
    ...
    kritikEvrak,
    nearestCritical,
    nearestCriticalLabel: formatNearestCriticalLabel(nearestCritical),
    hasData:
      muayene + sigorta + bakim + lastik + kritikEvrak > 0 ||
      Boolean(nearestCritical) ||
      (bundle.vehicleCount || 0) > 0,
  };
}

function upcomingWorkCenterHtml(context) {
  const metric = (label, value, tone = "neutral") =>
    `<article class="upcoming-work-center__metric upcoming-work-center__metric--${tone}">
      <span class="upcoming-work-center__metric-label">${escapeHtml(label)}</span>
      <strong class="upcoming-work-center__metric-value">${Number(value || 0).toLocaleString("tr-TR")}</strong>
    </article>`;

  return `<section class="upcoming-work-center fade-in" aria-label="Yaklaşan işler merkezi">
    <header class="upcoming-work-center__head">
      <div>
        <h2 class="upcoming-work-center__title">Yaklaşan İşler Merkezi</h2>
        <p class="upcoming-work-center__subtitle">Operasyonel öncelik ve uygunluk takibi</p>
      </div>
      <a href="/documents" class="btn btn--ghost btn--sm">Uygunluk →</a>
    </header>
    <div class="upcoming-work-center__body">
      <div class="upcoming-work-center__metrics">
        ${metric("Muayene", context.muayene, "info")}
        ${metric("Sigorta", context.sigorta, "info")}
        ${metric("Bakım", context.bakim, "warning")}
        ${metric("Lastik Değişimi", context.lastik, "warning")}
        ${metric("Kritik Evrak", context.kritikEvrak, "danger")}
      </div>
      <div class="upcoming-work-center__nearest">
        <span class="upcoming-work-center__nearest-label">En yakın kritik tarih</span>
        <strong class="upcoming-work-center__nearest-value">${escapeHtml(context.nearestCriticalLabel)}</strong>
      </div>
    </div>
  </section>`;
}

function dashboardFinanceTrendPanel() {
  return `<section class="cmd-panel cmd-panel--chart dashboard-finance-trend fade-in">
    <header class="cmd-panel__head">
      <div>
        <h3 class="cmd-panel__title">Gelir / Gider Trendi</h3>
        <p class="cmd-panel__desc">Son 6 ay gelir ve gider</p>
      </div>
      <a href="/reports" class="btn btn--ghost btn--sm">Analizler</a>
    </header>
    <div class="cmd-panel__body cmd-chart-area">
      <div class="chart-wrap chart-wrap--command trend-chart"><canvas id="monthlyChart"></canvas></div>
    </div>
  </section>`;
}
...
function miniFinanceTrendsPanel() {
  return `<section class="cmd-panel cmd-panel--chart mini-financial-trend fade-in">
    <header class="cmd-panel__head">
      <div>
