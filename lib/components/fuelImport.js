...
function fuelImportDualFormHtml(opts = {}) {
  const compact = !!(opts && opts.compact);
  const formCls = compact ? "fuel-import-form fuel-import-form--compact" : "fuel-import-form";
  const dualCls = compact ? "fuel-import-dual fuel-import-dual--compact" : "fuel-import-dual";
  const dropCls = compact ? " fuel-dropzone--compact" : "";
  const optsCls = compact ? " fuel-import-options--compact" : "";
  const btnCls = compact ? " btn--sm fuel-import-submit--compact" : "";
  const detailHint = compact
    ? "Dokum Excel — satır bazlı yakıt"
    : "Dokum-10.06.2026.xlsx — satır satır yakıt alımları";
  const controlHint = compact
    ? "Mutabakat — toplam doğrulama"
    : "Yakıt Alım Raporu — toplam litre/tutar doğrulama";

  return `<form method="POST" action="/fuel/import" enctype="multipart/form-data" class="${formCls}">
    <div class="${dualCls}">
      <label class="fuel-dropzone fuel-dropzone--detail${dropCls}">
        <input type="file" name="detailFile" accept=".xlsx,.xls" required />
        <strong>Detay Excel (Dokum)</strong>
        <span>${detailHint}</span>
      </label>
      <label class="fuel-dropzone fuel-dropzone--control${dropCls}">
        <input type="file" name="controlFile" accept=".xlsx,.xls" />
        <strong>Kontrol / Mutabakat Excel (opsiyonel)</strong>
        <span>${controlHint}</span>
      </label>
    </div>
    <div class="fuel-import-options${optsCls}">
      <label class="filter-check fuel-import-opt">
        <input type="checkbox" name="auto_create_vehicle" value="1"/> Eşleşmeyen plakalar için otomatik araç oluştur
      </label>
      <label class="filter-check fuel-import-opt">
    ...
    <button type="submit" class="btn btn-primary${btnCls}">İçe Aktar</button>
  </form>`;
}
