  ...
  const body = sorted
    .map((r) => {
      ...
      return `<tr class="profit-table__row">
        ...
        <td>${money(r.subcontractor || 0)}</td>
        <td>${money(r.personnel || 0)}</td>
        <td>${money(r.payrollAllocated || 0)}</td>
    ...
    <table class="profit-table">
      <thead>
        <tr>
          ...
          <th>Taşeron</th>
          <th>Personel</th>
          <th>Bordro Dağıtım</th>
          <th>Toplam Gider</th>
          <th>Net Kâr</th>
          <th>Karlılık %</th>
        </tr>
      </thead>
      <tbody>${body || `<tr><td colspan="11">Kayıt yok</td></tr>`}</tbody>
    </table>
  </div>`;
}
...
function profitabilityFleetOverhead(summary) {
  const generalPayroll = summary.generalPayrollAllocationExpense || 0;
  const unassignedPersonnel = summary.unassignedPersonnelExpense || 0;
  const unassignedSub = summary.unassignedSubcontractorExpense || 0;
  if (!generalPayroll && !unassignedPersonnel && !unassignedSub) return "";

  return `<div class="profit-fleet-overhead fade-in">
    <article class="profit-fleet-overhead__item">
      <span>Genel Personel Yükü (SGK/Muhtasar)</span>
      <strong>${money(generalPayroll)}</strong>
    </article>
    <article class="profit-fleet-overhead__item">
      <span>Atanmamış Personel Gideri</span>
      <strong>${money(unassignedPersonnel)}</strong>
    </article>
    <article class="profit-fleet-overhead__item">
      <span>Atanmamış Taşeron Gideri</span>
      <strong>${money(unassignedSub)}</strong>
    </article>
  </div>`;
}
...
function profitabilityPage({ summary, rows, rankedRows, hasData, vehicleFilter = "" }) {
  ...
  return `<div class="dash page-enter profit-hub">
    <header class="profit-hub__header fade-in">
      ...
      <h2 class="profit-hub__title">Araç Karlılık Merkezi</h2>
      <p class="profit-hub__desc">Net Kâr = Gelir − (Yakıt + HGS/OGS + Bakım + Taşeron + Personel + Bordro Dağıtım + Diğer) · Genel personel yükü filo toplamına dahildir</p>
    </header>
    ${profitabilityFilterBar(vehicleFilter)}
    ${profitabilityKpiRow(summary)}
    ${profitabilityFleetOverhead(summary)}
