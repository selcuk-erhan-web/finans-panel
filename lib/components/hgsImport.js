...
function hgsImportFormHtml() {
  // input name="pdfFile" must match upload.single("pdfFile") in routes/hgs.js
  return `<form id="hgsPdfImportForm" method="POST" action="/hgs/import" enctype="multipart/form-data" class="fuel-import-form hgs-import-form">
    <div class="hgs-upload-field">
      <label for="hgsPdfFileInput" class="hgs-upload-label">HGS PDF dosyası</label>
      <input
        type="file"
        name="pdfFile"
        id="hgsPdfFileInput"
      ...
      />
      <p class="hgs-upload-hint">Sadece İş Bankası HGS ekstre PDF formatı desteklenir · max 20 MB</p>
    </div>
    <button type="submit" class="btn btn-primary">PDF İçe Aktar</button>
  </form>
  <style>
    .hgs-upload-field { display: flex; flex-direction: column; gap: 8px; }
    .hgs-upload-label { font-weight: 700; font-size: 14px; color: #0f172a; }
    .hgs-upload-input {
      display: block;
      width: 100%;
      max-width: 100%;
    ...
    .hgs-upload-hint { margin: 0; font-size: 13px; color: #64748b; }
  </style>`;
}
