...
function alertCardHtml(alert) {
  ...
  return `<article class="corp-alert corp-alert--${meta.tone}">
    ...
    <div class="corp-alert__body">
      ...
      ${alert.daysLeft != null ? `<p class="corp-alert__meta">${Number(alert.daysLeft).toLocaleString("tr-TR")} gün</p>` : ""}
    </div>
...
function alertsPageHtml(summary) {
  ...
  const kpiRow = `<div class="corp-alert-summary-row fade-in">
  ...
  if (!total) {
    return `<div class="dash page-enter dash--dense alerts-hub">
    ...
    ${kpiRow}
    <section class="alerts-empty-state fade-in">
      <div class="alerts-empty-state__icon" aria-hidden="true">✓</div>
      <h3 class="alerts-empty-state__title">Şu anda aktif operasyonel risk bulunmuyor.</h3>
      <p class="alerts-empty-state__desc">Filo, gelir-gider ve personel verileri güncel kaldığında anomaliler burada listelenir.</p>
      <div class="alerts-empty-state__actions">
        <a href="/" class="btn btn--ghost btn--sm">Ana Ekran</a>
        <a href="/vehicles" class="btn btn--ghost btn--sm">Araçlar</a>
      </div>
    </section>
  </div>`;
  }
  ...
  return `<div class="dash page-enter dash--dense alerts-hub">
    ...
    ${alertSectionHtml(
      "Evrak Uyarıları",
      "Muayene, sigorta ve yetki belgesi bitiş tarihleri",
      byType.DOCUMENT_EXPIRY,
      "Yaklaşan evrak uyarısı yok."
    )}
    ${alertSectionHtml(
      "Hakediş / Tahsilat Uyarıları",
      "Beklenen gelir ile hakediş farkları",
      byType.RECON_UNDERPAYMENT,
      "Eksik tahsilat uyarısı yok."
    )}
    ${alertSectionHtml(
      "Taşeron Gider Uyarıları",
      "Araç veya hat ile eşleşmemiş taşeron ödemeleri",
      byType.SUBCONTRACTOR_UNASSIGNED_COST,
      "Atanmamış taşeron gideri yok."
    )}
    ${alertSectionHtml(
      "Personel Gider Uyarıları",
      "Araç atanmamış personel maliyetleri",
      byType.PERSONNEL_UNASSIGNED_COST,
      "Atanmamış personel gideri yok."
    )}
    ${alertSectionHtml(
      "SGK / Muhtasar Uyarıları",
      "Vadesi yaklaşan veya geciken bordro yükümlülükleri",
      byType.PAYROLL_OBLIGATION_DUE,
      "SGK/Muhtasar uyarısı yok."
    )}
    ${alertSectionHtml(
      "Personel Yük Dağıtım Uyarıları",
      "Kârlılığa henüz dağıtılmamış SGK/Muhtasar tahakkukları",
      byType.PAYROLL_UNALLOCATED_OBLIGATION,
      "Dağıtılmamış personel yükü yok."
    )}
    ${alertSectionHtml(
      "Nakit Akışı Uyarıları",
      "Beklenen nakit açığı ve kısa vadeli baskı",
      byType.CASHFLOW_RISK,
      "Nakit akışı riski tespit edilmedi."
    )}
  </div>`;
}
