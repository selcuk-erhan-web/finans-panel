...
function vehicleIntelligencePageHtml(payload) {
  ...
  const rows = vehicles.length
    ? vehicles
        ...
        .join("")
    : `<tr><td colspan="9" class="data-table__empty">Araç zekâsı için veri bulunmuyor.</td></tr>`;

  return `<div class="dash page-enter dash--dense vi-hub">
    <header class="vi-hub__header fade-in">
      <p class="vi-hub__eyebrow">Filo · Vehicle Intelligence</p>
      <h2 class="vi-hub__title">Araç Zekâsı</h2>
      <p class="vi-hub__desc">Uygunluk, bakım, lastik, denetim ve finans verilerinin araç bazlı birleşik görünümü.</p>
    </header>

    <section class="vi-kpi-row fade-in">
      <article class="vi-kpi"><span>Toplam Araç</span><strong>${Number(summary.total_vehicles || 0).toLocaleString("tr-TR")}</strong></article>
      <article class="vi-kpi vi-kpi--crit"><span>Kritik Araç</span><strong>${Number(summary.vehicles_with_critical_signals || 0).toLocaleString("tr-TR")}</strong></article>
      <article class="vi-kpi vi-kpi--warn"><span>Uyarılı Araç</span><strong>${Number(summary.vehicles_with_warning_signals || 0).toLocaleString("tr-TR")}</strong></article>
      <article class="vi-kpi vi-kpi--ok"><span>Kârlı Araç</span><strong>${Number(summary.vehicles_profitable || 0).toLocaleString("tr-TR")}</strong></article>
      <article class="vi-kpi vi-kpi--neg"><span>Zararlı Araç</span><strong>${Number(summary.vehicles_unprofitable || 0).toLocaleString("tr-TR")}</strong></article>
      <article class="vi-kpi"><span>Toplam Gelir</span><strong>${money(summary.total_income || 0)}</strong></article>
      <article class="vi-kpi"><span>Toplam Gider</span><strong>${money(summary.total_expense || 0)}</strong></article>
      <article class="vi-kpi vi-kpi--net"><span>Net Kâr</span><strong>${money(summary.net_profit || 0)}</strong></article>
    </section>

    <section class="panel fade-in">
      <header class="panel__head">
        <h2 class="panel__title">Araç Zekâsı Tablosu</h2>
        <p class="panel__desc">Kritik sinyaller önce; net kâr ve plaka sıralaması uygulanır.</p>
      </header>
      <div class="panel__body">
        <div class="table-wrap">
          <table class="data-table data-table--compact vi-table">
            <thead>
              <tr>
                <th>Araç</th>
                <th>Uygunluk</th>
