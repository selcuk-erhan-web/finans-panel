...
function maintenanceCenterPageHtml({ summary, rows, vehicles, filters, editRecord, selectedVehiclePlate }) {
  ...
  const summaryCards = `
    <div class="mnt-kpi-row fade-in">
      ...
      <article class="mnt-kpi mnt-kpi--vehicles"><span>${filterActive ? "Son Bakım" : "Bakımlı Araç"}</span><strong>${filterActive ? (summary.last_maintenance_date ? formatDateDisplay(summary.last_maintenance_date) : "—") : Number(summary.vehicles_with_maintenance).toLocaleString("tr-TR")}</strong></article>
    </div>`;
  ...
  return `<div class="dash page-enter dash--dense maintenance-hub">
    <header class="maintenance-hub__header fade-in">
      ...
      <p class="maintenance-hub__desc">${filterActive ? `${escapeHtml(selectedVehiclePlate || "Seçili araç")} bakım geçmişi` : "Araç bakım geçmişi ve maliyet takibi"}</p>
    </header>
    ...
    <section class="panel fade-in">
      <header class="panel__head">
        <div>
          ...
          <p class="panel__desc">${rows.length} kayıt · en yeni önce${filterActive ? " · araç filtresi aktif" : ""}</p>
        </div>
