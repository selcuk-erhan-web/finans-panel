const { escapeHtml } = require("./escape");
const { formatDateDisplay } = require("../../utils/date");
const { formatMoneyInputValue, moneyInputHtml } = require("../../utils/money");
const { money } = require("../finance");
const { MAINTENANCE_TYPES } = require("../constants");
const { typeLabel } = require("../../services/maintenanceService");
const { entityAuditHistoryLink } = require("./auditLogs");
const { renderModuleTabs } = require("./moduleTabs");
const { executiveKpi, executiveKpiGrid, executiveHubHeader } = require("./executiveDesign");
const { buildMaintenanceIntelligence, maintenanceIntelligencePanel } = require("./executiveIntelligence");

function maintenanceTypeOptions(selected = "") {
  const keys = new Set(MAINTENANCE_TYPES.map(([k]) => k));
  if (selected && !keys.has(selected)) keys.add(selected);
  return [...keys]
    .map((k) => {
      const label = typeLabel(k);
      return `<option value="${k}" ${k === selected ? "selected" : ""}>${escapeHtml(label)}</option>`;
    })
    .join("");
}

function maintenanceHistorySummaryHtml(summary) {
  return `<div class="mnt-history-summary">
    <article class="mnt-history-summary__item"><span>Toplam Kayıt</span><strong>${Number(summary.total_records).toLocaleString("tr-TR")}</strong></article>
    <article class="mnt-history-summary__item"><span>Toplam Maliyet</span><strong>${money(summary.total_cost)}</strong></article>
    <article class="mnt-history-summary__item"><span>Son Bakım</span><strong>${summary.last_maintenance_date ? formatDateDisplay(summary.last_maintenance_date) : "—"}</strong></article>
    <article class="mnt-history-summary__item"><span>Son KM</span><strong>${summary.last_odometer_km != null ? Number(summary.last_odometer_km).toLocaleString("tr-TR") : "—"}</strong></article>
  </div>`;
}

function maintenanceHistoryRowsHtml(records, opts = {}) {
  const { limit } = opts;
  const rows = limit ? records.slice(0, limit) : records;

  if (!rows.length) {
    return `<tr><td colspan="6" class="data-table__empty">Bu araç için bakım kaydı bulunmuyor.</td></tr>`;
  }

  return rows
    .map(
      (row) => `<tr>
        <td>${formatDateDisplay(row.maintenance_date)}</td>
        <td>${row.odometer_km != null ? Number(row.odometer_km).toLocaleString("tr-TR") : "—"}</td>
        <td>${escapeHtml(row.maintenance_type_label || typeLabel(row.maintenance_type))}</td>
        <td>${row.cost ? money(row.cost) : "—"}</td>
        <td>${escapeHtml(row.vendor || "—")}</td>
        <td>${escapeHtml(row.description || "—")}</td>
      </tr>`
    )
    .join("");
}

function maintenanceCenterPageHtml({ summary, rows, vehicles, filters, editRecord, selectedVehiclePlate, scheduleReport = null, path = "/maintenance" }) {
  const vehicleFilter = filters.vehicle_id || "";
  const selectedVehicle = editRecord ? String(editRecord.vehicle_id) : vehicleFilter;
  const selectedType = editRecord?.maintenance_type || "";
  const costValue =
    editRecord?.cost != null && editRecord.cost > 0 ? formatMoneyInputValue(editRecord.cost) : "";
  const filterActive = Boolean(vehicleFilter);

  const filterBanner = filterActive
    ? `<div class="mnt-filter-banner fade-in">
        <span>Filtre: <strong>${escapeHtml(selectedVehiclePlate || "Seçili araç")}</strong> · ${rows.length} kayıt</span>
        <a href="/maintenance" class="btn btn--ghost btn--sm">Tüm kayıtları göster</a>
        <a href="/vehicle/${escapeHtml(vehicleFilter)}" class="btn btn--ghost btn--sm">Araç Merkezi →</a>
      </div>`
    : "";

  const summaryCards = executiveKpiGrid(
    [
      executiveKpi({
        label: "Toplam Kayıt",
        value: Number(summary.total_records).toLocaleString("tr-TR"),
        tone: "neutral",
      }),
      executiveKpi({ label: "Toplam Maliyet", value: money(summary.total_cost), tone: "info" }),
      executiveKpi({
        label: filterActive ? "Son Bakım" : "Bakımlı Araç",
        value: filterActive
          ? summary.last_maintenance_date
            ? formatDateDisplay(summary.last_maintenance_date)
            : "—"
          : Number(summary.vehicles_with_maintenance).toLocaleString("tr-TR"),
        tone: "success",
      }),
    ].join("")
  );

  const emptyState = filterActive
    ? `<tr><td colspan="8" class="data-table__empty executive-table__empty">
        Bu araç için bakım kaydı bulunmuyor.
        <span class="data-table__empty-hint">İlk bakım kaydını oluşturmak için formu kullanın.</span>
      </td></tr>`
    : `<tr><td colspan="8" class="data-table__empty executive-table__empty">
        FleetOS bu alan için veri bekliyor
        <span class="data-table__empty-hint">İlk bakım kaydı oluşturulduğunda plan analizi otomatik oluşacaktır.</span>
      </td></tr>`;

  const tableRows = rows.length
    ? rows
        .map(
          (row) => `<tr>
          <td><a class="plate-link" href="/maintenance?vehicle_id=${row.vehicle_id}" title="Bu aracın bakım geçmişi">${escapeHtml(row.plate || "—")}</a></td>
          <td>${escapeHtml(row.maintenance_type_label || typeLabel(row.maintenance_type))}</td>
          <td>${formatDateDisplay(row.maintenance_date)}</td>
          <td>${row.odometer_km != null ? Number(row.odometer_km).toLocaleString("tr-TR") : "—"}</td>
          <td>${row.cost ? money(row.cost) : "—"}</td>
          <td>${escapeHtml(row.vendor || "—")}</td>
          <td>${escapeHtml(row.description || "—")}</td>
          <td class="data-table__actions">
            <a href="/maintenance/edit/${row.id}" class="btn btn--sm btn--ghost">Düzenle</a>
            <a href="/maintenance/delete/${row.id}" class="btn btn--sm btn--danger" onclick="return confirm('Bakım kaydı silinsin mi?')">Sil</a>
          </td>
        </tr>`
        )
        .join("")
    : emptyState;

  const formTitle = editRecord ? "Bakım kaydı düzenle" : "Bakım Kaydı";
  const formAction = editRecord ? `/maintenance/edit/${editRecord.id}` : "/maintenance/add";

  const formPanel = `<section class="panel executive-panel executive-panel--form fade-in maintenance-form-panel">
    <header class="panel__head executive-panel__head">
      <h2 class="panel__title executive-panel__title">${escapeHtml(formTitle)}</h2>
      ${
        editRecord
          ? entityAuditHistoryLink({
              module: "maintenance",
              entity_type: "maintenance_record",
              entity_id: editRecord.id,
            })
          : ""
      }
    </header>
    <div class="panel__body executive-panel__body">
      <form method="POST" action="${formAction}" class="form-grid executive-form maintenance-form">
        <label class="field-label">Araç</label>
        <select name="vehicle_id" required>
          <option value="">Araç seçin</option>
          ${vehicles
            .map(
              (v) =>
                `<option value="${v.id}" ${String(selectedVehicle) === String(v.id) ? "selected" : ""}>${escapeHtml(v.plate)}</option>`
            )
            .join("")}
        </select>

        <label class="field-label">Bakım Türü</label>
        <select name="maintenance_type" required>${maintenanceTypeOptions(selectedType)}</select>

        <label class="field-label">Tarih</label>
        <input type="date" name="maintenance_date" required value="${escapeHtml(editRecord?.maintenance_date || "")}"/>

        <label class="field-label">KM</label>
        <input type="number" name="odometer_km" min="0" step="1" placeholder="Kilometre" value="${editRecord?.odometer_km ?? ""}"/>

        <label class="field-label">Maliyet</label>
        ${moneyInputHtml("cost", {
          value: costValue,
          required: false,
          placeholder: "Maliyet (örn. 4.250,00)",
          className: "money-input",
        })}

        <label class="field-label">Servis / Tedarikçi</label>
        <input name="vendor" placeholder="Servis veya tedarikçi" value="${escapeHtml(editRecord?.vendor || "")}"/>

        <label class="field-label full">Açıklama</label>
        <input class="full" name="description" placeholder="İşlem açıklaması" value="${escapeHtml(editRecord?.description || "")}"/>

        <div class="form-actions full">
          <button type="submit" class="btn btn--primary">${editRecord ? "Güncelle" : "Kaydet"}</button>
          ${editRecord ? `<a href="/maintenance" class="btn btn--ghost">İptal</a>` : ""}
        </div>
      </form>
    </div>
  </section>`;

  const maintenanceIntel = buildMaintenanceIntelligence({
    scheduleReport,
    summary,
    vehicleCount: vehicles.length,
  });
  const maintenanceIntelPanel = maintenanceIntelligencePanel(maintenanceIntel);

  return `<div class="dash page-enter dash--executive executive-hub maintenance-hub">
    ${executiveHubHeader({
      eyebrow: "Filo · Bakım",
      description: filterActive
        ? `${selectedVehiclePlate || "Seçili araç"} bakım geçmişi`
        : "Araç bakım geçmişi ve maliyet takibi",
      tabsHtml: renderModuleTabs("maintenance", path),
    })}

    ${filterBanner}
    ${summaryCards}

    <div class="grid2 grid2--executive maintenance-hub__entry-grid">
      ${formPanel}
      ${maintenanceIntelPanel}
    </div>

    <section class="panel executive-panel executive-panel--table fade-in">
      <header class="panel__head executive-panel__head">
        <div>
          <h2 class="panel__title executive-panel__title">Bakım Kayıtları</h2>
          <p class="panel__desc executive-panel__subtitle">${
            rows.length
              ? `${rows.length} kayıt görüntüleniyor · en yeni önce${filterActive ? " · araç filtresi aktif" : ""}`
              : "Veri bekleniyor — ilk bakım kaydı sonrası liste dolacak"
          }</p>
        </div>
        <form class="filters" method="GET" action="/maintenance">
          <select name="vehicle_id" onchange="this.form.submit()">
            <option value="">Tüm araçlar</option>
            ${vehicles
              .map(
                (v) =>
                  `<option value="${v.id}" ${String(vehicleFilter) === String(v.id) ? "selected" : ""}>${escapeHtml(v.plate)}</option>`
              )
              .join("")}
          </select>
          <a href="/maintenance" class="btn btn--ghost btn--sm">Temizle</a>
          <a href="/export/maintenance/xlsx" class="btn btn--ghost btn--sm">Excel</a>
        </form>
      </header>
      <div class="panel__body executive-panel__body">
        <div class="table-wrap executive-table-wrap">
        <table class="data-table executive-table">
          <thead><tr>
            <th>Araç</th><th>Tür</th><th>Tarih</th><th>KM</th><th>Maliyet</th><th>Servis</th><th>Açıklama</th><th>İşlem</th>
          </tr></thead>
          <tbody>${tableRows}</tbody>
        </table>
        </div>
      </div>
    </section>
  </div>`;
}

module.exports = {
  maintenanceCenterPageHtml,
  maintenanceTypeOptions,
  maintenanceHistorySummaryHtml,
  maintenanceHistoryRowsHtml,
};
