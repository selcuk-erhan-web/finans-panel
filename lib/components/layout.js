...
const NAV_TOP = [
  ...
  ["/alerts", "alert", "Uyarılar"],
  ["/documents", "document", "Evrak Takibi"],
  ["/reconciliation", "reconcile", "Hakediş Kontrol"],
  ["/subcontractors", "subcontractor", "Taşeronlar"],
  ["/employees", "employee", "Personel"],
  ["/payroll", "payroll", "SGK / Muhtasar"],
  ["/cashflow", "wallet", "Nakit Akışı"],
];
...
function renderIncomeNavGroup(path) {
  ...
  return `<div class="nav-group nav-group--income${openClass}${groupActive}" data-nav-group="income">
    ...
    <div class="nav-group__sub" id="incomeNavSub">${subLinks}</div>
  </div>`;
}

function renderExpenseNavGroup(path) {
  ...
  const subLinks = EXPENSE_NAV_ITEMS.map((item) => {
    ...
    return `<a href="${item.href}" class="nav-link nav-link--sub nav-link--text nav-link--expense${isActive ? " is-active" : ""}">
      <span class="nav-link__label">${escapeHtml(item.label)}</span>
    </a>`;
  }).join("");

  return `<div class="nav-group nav-group--expense${openClass}${groupActive}" data-nav-group="expense">
    ...
    <div class="nav-group__sub" id="expenseNavSub">${subLinks}</div>
  </div>`;
}
...
function renderNav(path) {
  ...
  const expense = renderExpenseNavGroup(path);
...
<body>
  ...
  <div class="app-frame app-container">
    <aside class="sidebar" id="sidebar">
      ...
      <nav class="sidebar__nav" aria-label="Ana menü" data-sidebar-nav="fleetos-income-04-accordion">${nav}</nav>
