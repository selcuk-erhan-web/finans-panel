...
function resolveWorkspaceBodyClass(path) {
  const p = String(path || "").split("?")[0];
  if (p === "/") return "workspace__body workspace__body--cockpit";
  return "workspace__body workspace__body--scroll";
}

function shell({ title, subtitle, content, path, categorySlug, flash }) {
  const nav = renderNav(path);
  const flashScript = "";
  const workspaceBodyClass = resolveWorkspaceBodyClass(path);
  ...
  <div class="app-frame app-container">
    <aside class="sidebar" id="sidebar">
      <div class="sidebar__brand">
        <img
          src="/images/mistur-fleetos-logo-cropped.png"
          alt="MISTUR FleetOS"
    ...
    <div class="workspace">
      <header class="masthead">
        ...
        <div class="masthead__actions">
          <a href="/notifications" class="masthead__notifications" id="notificationsBadgeLink">
            Notifications (<span id="notificationsUnreadCount">0</span>)
          </a>
          <a href="/maintenance-alerts" class="masthead__maintenance-alerts" id="maintenanceAlertsBadgeLink">
            Bakım Uyarıları (<span id="maintenanceAlertsUnreadCount">0</span>)
          </a>
          <a href="/tire-alerts" class="masthead__tire-alerts" id="tireAlertsBadgeLink">
            Lastik Uyarıları (<span id="tireAlertsUnreadCount">0</span>)
          </a>
      ...
      <main class="${workspaceBodyClass}">${content}</main>
    </div>
  </div>
  <script src="/js/main.js?v=${LAYOUT_VERSION}"></script>
  <script src="/js/notifications-badge.js?v=${LAYOUT_VERSION}"></script>
  <script src="/js/maintenance-alerts-badge.js?v=${LAYOUT_VERSION}"></script>
  <script src="/js/tire-alerts-badge.js?v=${LAYOUT_VERSION}"></script>
</body>
</html>`;
}
