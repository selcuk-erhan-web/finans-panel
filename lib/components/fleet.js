...
function vehicleCard(v) {
  ...
  return `<article class="vehicle-card vehicle-card--${state}">
    <div class="vehicle-card__head">
      <span class="vehicle-card__plate">${escapeHtml(v.plate)}</span>
      ${typeBadge(v.type)}
    </div>
    ...
    <a href="/vehicle/${v.id}" class="vehicle-card__cta">
      Detay ${ICONS.arrow}
    </a>
  </article>`;
}
...
function fleetCardLarge(v) {
  ...
  return `<article class="fleet-card fleet-card--${state} fade-in">
    ...
    <div class="fleet-card__foot">
      ${statusPill(v)}
      <a href="/vehicle/${v.id}" class="btn btn--primary fleet-card__cta">
        Detay ${ICONS.arrow}
      </a>
    </div>
  </article>`;
...
function fleetCardGrid(summaries) {
  if (!summaries.length) {
    return `<div class="empty fade-in">
      <div class="empty__icon">🚗</div>
      <h3>Henüz araç yok</h3>
      <p>İlk aracınızı ekleyerek premium filo panelinizi oluşturun.</p>
    </div>`;
  }
  return `<div class="fleet-card-grid">${summaries.map(fleetCardLarge).join("")}</div>`;
}
