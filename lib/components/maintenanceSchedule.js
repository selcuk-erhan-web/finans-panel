...
function maintenanceSchedulePageHtml({ summary, schedules, vehicles, filters, selectedVehiclePlate }) {
  ...
  const summaryCards = `
    <div class="mnt-schedule-kpi-row fade-in">
      <article class="mnt-schedule-kpi mnt-schedule-kpi--ok"><span>OK</span><strong>${Number(summary.ok).toLocaleString("tr-TR")}</strong></article>
      <article class="mnt-schedule-kpi mnt-schedule-kpi--upcoming"><span>Yaklaşıyor</span><strong>${Number(summary.upcoming).toLocaleString("tr-TR")}</strong></article>
      <article class="mnt-schedule-kpi mnt-schedule-kpi--due"><span>Günü Geldi</span><strong>${Number(summary.due).toLocaleString("tr-TR")}</strong></article>
      <article class="mnt-schedule-kpi mnt-schedule-kpi--overdue"><span>Gecikti</span><strong>${Number(summary.overdue).toLocaleString("tr-TR")}</strong></article>
      <article class="mnt-schedule-kpi mnt-schedule-kpi--unknown"><span>Bilinmiyor</span><strong>${Number(summary.unknown).toLocaleString("tr-TR")}</strong></article>
    </div>`;
    ...
    <section class="panel fade-in">
      <header class="panel__head">
        <div>
          ...
          <p class="panel__desc">${summary.total} plan satırı · en kritik önce</p>
        </div>
