...
const { buildTireIntelligence, tireIntelligencePanel } = require("./executiveIntelligence");
...
function tireCenterPageHtml({ summary, rows, vehicles, filters, editRecord, selectedVehiclePlate, seasonalReport = null, path = "/tires" }) {
  ...
  const summaryCards = executiveKpiGrid(
    [
      executiveKpi({ label: "Toplam Kayıt", value: Number(summary.total_records).toLocaleString("tr-TR"), tone: "neutral" }),
      executiveKpi({ label: "Toplam Adet", value: Number(summary.total_quantity).toLocaleString("tr-TR"), tone: "info" }),
      executiveKpi({ label: "Araç Üzerinde", value: Number(summary.on_vehicle).toLocaleString("tr-TR"), tone: "success" }),
      executiveKpi({ label: "Depoda", value: Number(summary.in_storage).toLocaleString("tr-TR"), tone: "neutral" }),
      executiveKpi({ label: "Yazlık", value: Number(summary.summer).toLocaleString("tr-TR"), tone: "warning" }),
      executiveKpi({ label: "Kışlık", value: Number(summary.winter).toLocaleString("tr-TR"), tone: "info" }),
      executiveKpi({ label: "Toplam Maliyet", value: money(summary.total_cost), tone: "info" }),
    ].join("")
  );

  const emptyState = `<tr><td colspan="13" class="data-table__empty executive-table__empty">
      FleetOS bu alan için veri bekliyor.
      ${filterActive ? `<span class="data-table__empty-hint">Filtreleri temizleyin veya yeni kayıt ekleyin.</span>` : `<span class="data-table__empty-hint">İlk lastik kaydı oluşturulduğunda envanter analizi otomatik oluşacaktır.</span>`}
    </td></tr>`;
  ...
  const formPanel = `<section class="panel executive-panel executive-panel--form fade-in tire-form-panel">
    ...
    <div class="panel__body executive-panel__body">
      <form method="POST" action="${formAction}" class="form-grid executive-form tire-form">
  ...
  const tireIntel = buildTireIntelligence({
    seasonalReport,
    summary,
    vehicleCount: vehicles.length,
  });
  const tireIntelPanel = tireIntelligencePanel(tireIntel);

  return `<div class="dash page-enter dash--executive executive-hub tire-hub">
    ${executiveHubHeader({
      eyebrow: "Filo · Lastik",
      description: filterActive ? "Filtrelenmiş lastik envanteri" : "Araç lastik envanteri ve maliyet takibi",
      tabsHtml: renderModuleTabs("tire", path),
    })}

    ${filterBanner}
    ${summaryCards}

    <div class="grid2 grid2--executive tire-hub__entry-grid">
      ${formPanel}
      ${tireIntelPanel}
    </div>
    ...
    <section class="panel executive-panel executive-panel--table fade-in">
      <header class="panel__head executive-panel__head">
        <div>
          <h2 class="panel__title executive-panel__title">Lastik Kayıtları</h2>
          <p class="panel__desc executive-panel__subtitle">${
            rows.length ? `${rows.length} kayıt görüntüleniyor · en yeni önce` : "Veri bekleniyor — ilk lastik kaydı sonrası envanter dolacak"
          }</p>
        </div>
