...
function migrateAuditLogsFoundation() {
  [
...
function migrateTireCenterV1() {
  ...
  try {
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_tires_vehicle ON tires(vehicle_id)`).run();
  } catch (e) {}
  try {
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_tires_status ON tires(status)`).run();
  } catch (e) {}
  try {
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_tires_season ON tires(season)`).run();
  } catch (e) {}
}

function migrateTireHistoryV1() {
  db.prepare(`
  ...
  try {
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_tire_history_vehicle ON tire_change_history(vehicle_id)`).run();
  } catch (e) {}
  try {
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_tire_history_date ON tire_change_history(change_date)`).run();
  } catch (e) {}
  try {
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_tire_history_type ON tire_change_history(change_type)`).run();
  } catch (e) {}
}

function migrateTireAlertsV1() {
  db.prepare(`
    CREATE TABLE IF NOT EXISTS tire_alerts (
      ...
      required_tire_season TEXT,
      current_tire_season TEXT,
      message TEXT NOT NULL,
      status TEXT NOT NULL DEFAULT 'unread',
      source_key TEXT NOT NULL UNIQUE,
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  ...
  try {
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_tire_alerts_status ON tire_alerts(status)`).run();
  } catch (e) {}
  try {
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_tire_alerts_severity ON tire_alerts(severity)`).run();
  } catch (e) {}
}
